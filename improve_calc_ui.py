...
# The calc block currently starts with:
...
# Regex to find the existing .plan-calc and replace it entirely
html = re.sub(
    r'<div class="plan-calc" style="background:var\(--bg-sec\);.*?</div>\s*</div>\s*</div>',
    new_calc_block,
    html,
    flags=re.DOTALL
)
